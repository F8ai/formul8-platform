...
export async function registerRoutes(app: Express): Promise<Server> {
        ...
        const questionsWithResponses = await Promise.all(questions.map(async (q: any, index: number) => {
          ...
          for (const model of additionalModels) {
            ...
            if (!existingResponse) {
              baseQuestion.modelResponses.push({
                model: model,
                answer: 'Click "Run Test" to generate real response from this model',
                confidence: 0,
                grade: 0,
                gradingConfidence: 0,
                responseTime: 0,
  ...
  // API endpoint to run real model test on a specific question
  app.post('/api/agents/:agentType/baseline-questions/:questionId/test/:model', async (req, res) => {
    const { agentType, questionId, model } = req.params;
    
    try {
      // Load the baseline question
      const agentPath = path.join(process.cwd(), 'agents', `${agentType}-agent`);
      const baselineFile = path.join(agentPath, 'baseline.json');
      const baselineData = await fs.readFile(baselineFile, 'utf8');
      const baseline = JSON.parse(baselineData);
      
      const question = baseline.questions.find((q: any) => q.id === questionId);
      if (!question) {
        return res.status(404).json({ error: 'Question not found' });
      }
      
      // Run the real model test
      const result = await modelIntegrationService.testQuestion({
        id: question.id,
      ...
      console.error(`Error testing model ${model} on question ${questionId}:`, error);
  ...
  app.use('/api/agents', baselineQuestionsRoutes);
